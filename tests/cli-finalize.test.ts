...
describe('CLI: finalize UAT output', { timeout: 60000 }, () => {
  ...
  it('success: JSON payload carries uatPath into the archive plus all pre-existing fields; human mode prints the UAT script line', async () => {
    ...
    // Dry-run first (no archive write): the payload carries the default
    // enforcement value alongside the pre-existing dry-run fields.
    const dryRun = await runCli(['--json', 'finalize', '--dry-run', 'uat-success-json'], tempDir)
    expect(dryRun.code).toBe(0)
    const dryPayload = JSON.parse(dryRun.stdout) as Record<string, unknown>
    expect(dryPayload.status).toBe('dry_run')
    expect(dryPayload.uatEnforceOnShip).toBe(true)
    ...
    expect(payload.uatEnforceOnShip).toBe(true)
    ...
    // Enforcement on (the default) is silent in human mode.
    expect(humanRun.stdout).not.toContain('UAT enforcement:')
  ...
  it('uat.enabled false + enforce_on_ship false: uatPath null, uatEnforceOnShip false, no uatWarning key, human enforcement-off line', async () => {
    await installFixture(tempDir)
    await disableWorktrees(tempDir)
    ...
    configDoc.uat = {
      ...(configDoc.uat as Record<string, unknown> | undefined),
      enabled: false,
    ...
    // Configured enforce_on_ship: false is reflected even with UAT disabled.
    expect(payload.uatEnforceOnShip).toBe(false)
    expect('uatWarning' in payload).toBe(false)
    ...
    // Human mode only speaks when enforcement is off.
    expect(humanRun.stdout).toContain('UAT enforcement: off')
    expect(humanRun.stderr).not.toContain('Warning: UAT generation failed')
  })
