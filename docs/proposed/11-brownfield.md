...
The constitution is generated with the corrections applied. The user reviews the final `spec/project.md` and approves.
...
5. Output
   → spec/specs/auth/spec.md (draft — marked for review)
   → spec/specs/auth/spec.lock (content hashes + reconciliation status)
   → spec/gaps/<gap-name>.md (one file per gap)
```

### Gaps — Individual Files Per Gap

After import, Metta writes one file per gap in `spec/gaps/`. Each gap is a self-contained document — easy to track, review, assign, and promote into a spec.

```
spec/gaps/
  payments-partial-refunds.md
  notifications-push-notifications.md
  auth-password-reset-sms.md
  payments-annual-billing.md
  auth-rate-limiting-undocumented.md
  auth-session-expiry-divergence.md
```

#### Example: Claimed But Not Built

```markdown
<!-- spec/gaps/payments-partial-refunds.md -->
...
## Source
GSD REQUIREMENTS.md, Phase 3 marked "complete"

## Claim
"The system MUST support full and partial refunds"

## Evidence
Only full refunds implemented (src/app/api/refunds/route.ts).
No partial refund endpoint, amount validation, or partial refund tests found.

## Impact
Users cannot request partial refunds despite spec claiming support.

## Related Spec
spec/specs/payments/spec.md — Requirement: Refund Processing

## Action
Promote to spec: `metta propose --from-gap payments-partial-refunds`
```

#### Example: Built But Not Documented

```markdown
<!-- spec/gaps/auth-rate-limiting-undocumented.md -->
<!-- Generated by metta import — 2026-04-05 -->

# Gap: Rate Limiting (Undocumented)
...
## Behavior
100 requests/minute per IP, 429 response with Retry-After header.

## Test Coverage
2 tests in tests/rate-limit.test.ts

## Action
Add requirement to auth spec, or create separate spec:
`metta propose --from-gap auth-rate-limiting-undocumented`
```

#### Example: Divergence

```markdown
<!-- spec/gaps/auth-session-expiry-divergence.md -->
<!-- Generated by metta import — 2026-04-05 -->

# Gap: Session Expiry Divergence
...
## Code Does
Sessions expire after 12 hours (SESSION_TTL = 43200 in src/lib/auth/config.ts)

## Action
Update spec to match code, or fix code to match spec.
```
...
| Status | Meaning |
|--------|---------|
| `claimed-not-built` | Spec says it exists. Code says it doesn't. |
| `partial` | Some scenarios implemented, others missing. |
| `built-not-documented` | Code exists with no corresponding spec. |
| `diverged` | Spec says one thing, code does another. |

#### Listing and Managing Gaps
...
Phase 2: Ingest
  ...
  ✓ GSD artifacts mapped:
    - PROJECT.md → spec/project.md (constitution seed)
    - REQUIREMENTS.md → spec extraction
    - 4 phase summaries → spec/archive/
...
|-------------|----------------|
| `PROJECT.md` | `spec/project.md` (constitution seed) |
| `REQUIREMENTS.md` | `spec/specs/` (spec extraction) |
| `ROADMAP.md` | Workflow planning context |
| `.planning/N-CONTEXT.md` | Discovery context |
| `.planning/N-*-PLAN.md` | `spec/archive/` (historical reference) |
| `.planning/N-*-SUMMARY.md` | `spec/archive/` |

| Source (Spec Kit) | Target (Metta) |
|------------------|----------------|
| `specs/{branch}/spec.md` | `spec/specs/<capability>/spec.md` |
| `specs/{branch}/plan.md` | `spec/archive/` |
| `specs/{branch}/tasks.md` | `spec/archive/` |
| Constitution | `spec/project.md` (constitution seed) |

| Source (OpenSpec) | Target (Metta) |
|------------------|----------------|
| `openspec/specs/<capability>/spec.md` | `spec/specs/<capability>/spec.md` |
| `openspec/changes/<name>/` | `spec/changes/` or `spec/archive/` |
| `.openspec.yaml` | Change metadata |

| Source (BMAD) | Target (Metta) |
|--------------|----------------|
| `PRD.md` | `spec/specs/` (spec extraction) |
| `architecture.md` | `docs/architecture.md` |
| `epics.md` + stories | `spec/archive/` |
| `_bmad/bmm/config.yaml` | `spec/project.md` (constitution seed) |

| Source (Taskmaster) | Target (Metta) |
|--------------------|----------------|
| `docs/prd.md` | `spec/specs/` (spec extraction) |
| `tasks/tasks.json` | `spec/archive/` |
...
Phase 1: Import artifacts
  ✓ PROJECT.md → spec/project.md (constitution draft)
