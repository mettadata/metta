...
The context engine loads, budgets, and formats file-based context for AI agents operating within the metta change lifecycle. It provides per-artifact-type context manifests, token-budget enforcement, content caching, section extraction, and structured instruction generation.
...
### 4.2 Missing File Handling

Given a required source file that does not yet exist on disk,
When `resolve` is called,
Then the system MUST silently skip that file (no error thrown), because artifacts may not exist at every workflow stage.

Given an optional source file that does not exist on disk,
When `resolve` is called,
...
3. Check the in-memory cache. If the cached hash matches, use cached token count and content.
4. If not cached (or hash mismatch), count tokens with `countTokens` and populate the cache.
5. If `tokens > budgetRemaining`, truncate content and mark `truncated = true`.
6. Record the file path, content (possibly truncated), token count, hash, ISO-8601 load timestamp, truncation flag, and loading strategy.

### 5.2 Truncation

When a file's token count exceeds `budgetRemaining`, the system MUST:
...
## 6. Loading Strategy Selection

The system MUST assign a `strategy` to every `LoadedFile` based on the file's raw (pre-truncation) token count:
...
Note: the strategy field is informational metadata. The engine does not change loading behavior based on strategy; callers may use it to select post-processing (section extraction or skeleton generation).
...
The context engine MUST NOT throw when:
- A required or optional source file does not exist.
- The budget is zero or negative (all files are effectively skipped).

The context engine MAY throw when:
- A file exists but cannot be read (permissions, I/O error). Callers SHOULD handle this at the `resolve` level; currently, both required and optional file loading silently swallows all errors including I/O failures (see Gap: `gap-001`).
