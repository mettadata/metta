...
- Residual threat (accepted, documented): within a token's TTL, the orchestrator can call any subcommand in that skill's scope, not only the one the skill intended at that moment — bounded by per-skill scoping + TTL + audit logging.
