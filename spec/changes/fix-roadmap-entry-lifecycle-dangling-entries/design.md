...
| **Retire-hit pre-dirty accept**: when the resolved slug IS roadmapped and `spec/roadmap.md` carries uncommitted user edits, the auto-retire hook's canonical rewrite + conditional staging (`backlog done` and `fix-issue --remove-issue`) folds those pre-existing uncommitted edits into the archive commit — the whole-file `RoadmapStore` write cannot preserve unrelated dirty hunks, and any stray non-entry lines in the pre-dirty file are dropped by the canonical rewrite | Low | Accepted, not mitigated further: the store's whole-file canonical write is the pre-existing contract for every `RoadmapStore` mutation (`add`/`reorder`/`remove` all rewrite the whole file today), and the resulting archive commit is fully inspectable — remedy is `git show`/`git revert` on that commit. Recorded here rather than reworked because a partial/hunk-aware roadmap write is out of scope for this change. |
