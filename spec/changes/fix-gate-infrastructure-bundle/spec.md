...
## ADDED: Requirement: Gate execution MUST honor all three `on_failure` policies uniformly
...
- `retry_once` — on initial `fail`, re-run the gate once; return the second result.
- `continue_with_warning` — on `fail`, downgrade the returned status to `warn` (preserving `output`/`failures` for surfacing).
- `stop` — on `fail`, return the `fail` result unchanged AND signal to the caller that subsequent gates in the same batch MUST be skipped.
...
## ADDED: Requirement: Gate `warn` status MUST be treated as pass across CLI commands
...
Specifically:
- `Finalizer.finalize` MUST treat `warn` as pass (already current behavior).
- `metta verify` MUST treat `warn` as pass (changed from its prior fail-on-warn behavior).
- `merge-safety` gate checks MUST treat `warn` as pass.
...
## ADDED: Requirement: Workflow YAMLs MUST NOT reference unimplemented gates
...
## ADDED: Requirement: GateRegistry tests MUST cover each policy branch

Unit tests in `tests/gate-registry.test.ts` (or equivalent) MUST assert:
- `retry_once` retries exactly once on fail, does not retry on pass
- `continue_with_warning` translates fail to warn with preserved output
- `stop` causes subsequent gates in the same `runAll` batch to be marked `skip`
- `runAll` returns full-length arrays regardless of early stop
- `runWithPolicy` and `runWithRetry` produce identical results for a gate with `on_failure: retry_once` (back-compat alias)
