# Spec: branch-safety-guard-metta-issue-metta-backlog-state-mutating

## ADDED: Requirement: assert-on-main-branch-helper
...
- Return silently if the current branch equals `mainBranchName`
- Return silently if `overrideBranch` is supplied and equals the current branch
- Otherwise throw an Error whose message contains both the current branch name and the main branch name and starts with `Refusing to write: current branch`
- If git is not available or the cwd is not a git repo, the helper MUST return silently (non-git projects pass the guard)
...
## ADDED: Requirement: issue-and-backlog-commands-use-branch-guard
