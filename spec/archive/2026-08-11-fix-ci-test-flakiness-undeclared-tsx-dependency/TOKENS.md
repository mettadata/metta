# Token usage: fix-ci-test-flakiness-undeclared-tsx-dependency

- **Change**: fix-ci-test-flakiness-undeclared-tsx-dependency
- **Generated**: 2026-08-11

> Figures below are approximate, orchestrator-reported token counts collected
> during the change lifecycle. They are indicative of relative effort only —
> not billing-grade accounting — and may under- or over-count actual provider
> usage.

## Total

**~0 tokens** across 0 record(s).

## Per artifact

_No token usage recorded._

## Per role

_No token usage recorded._

## Per model

_No token usage recorded._

## Cheap/pinned (non-inherit) vs inherit

- **Cheap/pinned (non-inherit)**: ~0 tokens
- **Inherit**: ~0 tokens

## Gaps

- `design` — timed artifact with no reported token usage
- `implementation` — timed artifact with no reported token usage
- `intent` — timed artifact with no reported token usage
- `research` — timed artifact with no reported token usage
- `spec` — timed artifact with no reported token usage
- `stories` — timed artifact with no reported token usage
- `tasks` — timed artifact with no reported token usage
- `verification` — timed artifact with no reported token usage
