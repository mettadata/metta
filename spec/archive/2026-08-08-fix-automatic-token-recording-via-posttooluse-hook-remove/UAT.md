...
- **Runner**: inline (session subagent cap 200/200 reached; orchestrator executed the runner contract directly, per fallback directive)
- **Method**: live session evidence (a full day of hook-active fix pipelines), synthetic SubagentStop payload replay against the installed hook, archived TOKENS.md inspection, and machine-verified annotations backed by finalize/CI gates
- **Results**: 42 pass / 1 fail / 1 skip
- **FAILED — Step 1.2**: "TOKENS.md shows one entry per subagent run with no runs missing" — every TOKENS.md finalized 2026-08-11 has 0 records despite real metta-* subagent spawns. Root cause is environmental, not the hook code: the hook shells out to the globally-linked `metta`, whose main-checkout dist/ was stale (pre worktree-aware change resolution), so each recording attempt failed "No active changes" and the fail-open hook exited silently. Logged as issue `hooks-and-statusline-execute-stale-main-checkout-dist-via` (major). Synthetic replay against a rebuilt dist confirms the pipeline records correctly.
- **Skipped — Step 2.2**: negative research branch (usage not exposed) never applied — the spike confirmed SubagentStop exposes usage, so the stop-condition path is vacuous.
- **Notes**: Gaps section correctly flags each missing artifact ("timed artifact with no reported token usage") — observed in all empty 2026-08-11 reports; prose-era report (2026-08-08 enforce-pr change, 3 records) renders correctly; guard/mint hooks operated unaffected alongside the recording hook all session (5.2 observed live).
