...
- **capability-not-found** — a MODIFIED, RENAMED, or REMOVED delta targeting a capability with no spec on disk MUST produce a conflict; new-capability standing for an ADDED delta targeting a capability with no spec on disk is enforced upstream, at `metta complete spec`, via `SpecTargetError` — the merger itself creates the capability unconditionally for such a delta and does not re-check standing;
