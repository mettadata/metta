...
Implementation note: the shipped tests keep the shared PATH shim always-empty (`{"changes":[],"message":"No active changes"}`) and override it to "active" only inside the single test case that needs an active-change answer (the worktree-aggregation case above). This is a recorded Rule-1 deviation in `summary.md` — under V1c both the worktree and its hosting checkout derive the same probe root, so a single shim swap covers both without needing per-test shim variants elsewhere.
...
1. **Residual allow-surface widening: stale worktrees (ADR-3).** An edit into `.metta/worktrees/<old-abandoned>/` is allowed while any change is active at the host (pre-fix: blocked). *Mitigation:* bounded to metta-created directories under the probed checkout only — unrelated checkouts provably still block (test case 4). Accepted per intent ("intended behavior for the reproduced topology"); name-match hardening logged as issue `guard-edit-worktree-name-match-hardening-follow-up` in `spec/issues/` (the orchestrator logs it in the main checkout) together with Option 3 Part A (creation-time state commit), not shipped here.
...
3. **Path-shape false negatives in `deriveProbeRoot`** (a repo deliberately created at a path ending `…/.metta/worktrees/<x>` that is *not* a metta worktree): the hook would probe its grandparent's parent; a non-metta host makes the probe fail → fail open. *Mitigation:* only constructible by manually placing a git repo inside a directory literally named `.metta/worktrees/` — a metta-owned namespace; general nested-worktree support is explicitly out of scope per intent. No action.
   ...
   There is a sibling case this analysis has not previously called out: a plain git repo manually placed at `<X>/.metta/worktrees/<anything>`, where `X` is itself a metta project with an active change. This is **not** a fail-open case — the probe at `X` succeeds and returns `X`'s active change, so the manually-placed repo is *allowed by borrowing X's change*, same as any legitimate worktree would be. Constructing this requires write access to `X`'s `.metta/` directory, which is the same trust domain the fail-open philosophy (R4) already concedes — anyone who can write there can already fabricate `.metta/config.yaml` or `spec/changes/` state directly. Note also that the hook guards only the `Edit`/`Write`/`NotebookEdit` tool set, not `Bash`, so this path grants no privilege beyond what an attacker with that write access already has via a shell.
