...
## ADDED: Requirement: `metta propose` MUST accept a `--stop-after <artifact>` option
...
## ADDED: Requirement: `--stop-after` MUST be validated against the resolved workflow
...
1. Any value that is not a member of the workflow's `buildOrder`.
2. Any value that names an execution-phase artifact id (`implementation` or `verification`); these are explicitly forbidden because "stopping after" them is equivalent to running the full workflow and adds no value.
...
## ADDED: Requirement: change-record schema MUST persist `stop_after` as an optional field
...
## ADDED: Requirement: propose skill MUST honor the `stop_after` boundary

`.claude/skills/metta-propose/SKILL.md` MUST be updated so the orchestrator:

1. Parses an optional `--stop-after <artifact>` token from `$ARGUMENTS` using the same parse-and-strip pattern as `--workflow` and `--auto`.
2. Passes the flag through to `metta propose` when present.
3. After every `METTA_SKILL=1 metta complete <artifact>` call in Step 3 (per-artifact planning loop), inspects the change's persisted `stop_after` value. When the just-completed artifact id equals `stop_after`, the orchestrator MUST:
   a. Skip Step 4 (research synthesis — only relevant if `stop_after` is past `research`),
   b. Skip Step 5 (implementation),
   c. Skip Step 6 (review),
   d. Skip Step 7 (verification),
   e. Skip Step 8 (finalize / merge),
   f. Print exactly one handoff line in the form `Stopped after \`<artifact>\`. Run \`<resume-command>\` to <next-action>.`,
   g. Return to the user.
...
## ADDED: Requirement: `metta status` MUST surface `stop_after` in JSON output
...
## ADDED: Requirement: `--stop-after` MUST compose with all existing propose flags
...
## ADDED: Requirement: handoff message MUST be deterministic and matchable
...
- Test assertions that match the line literally.
- Future tooling to grep or parse the boundary signal.
- Users to recognize the boundary at a glance.
