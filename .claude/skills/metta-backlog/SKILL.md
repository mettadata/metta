...
2. Dispatch per choice:
   ...
   - **add** → ask for `title` (free-form), `priority` (`high | medium | low`), and `description` (free-form). First run `metta backlog list --json` (allow-listed; lets the session-credential mint hook complete a prior Bash cycle — output can be ignored), then run `metta backlog add "<title>" --priority <level> --description "<description>"`. Omit `--priority` if the user declined to pick one. Omit `--description` if the user left it blank (description then defaults to the title).
