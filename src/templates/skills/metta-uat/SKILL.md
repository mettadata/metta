...
1. **Resolve the target UAT.md and its change root.** This skill runs outside the artifact-instruction flow, so no instruction payload supplies `change_root` — resolve the change root per target and anchor every path and git command below with it: a **live** change's root is its worktree at `.metta/worktrees/<name>/` when that directory exists, otherwise the main checkout root; an **archived** document's root is always the main checkout root (archives live on main). Never assume the session cwd is the change root.
   - **Named argument** (`$ARGUMENTS` contains a change name): check `<change-root>/spec/changes/<name>/UAT.md` first (Read/Glob, `<change-root>` resolved as above for a live change); if absent, glob `spec/archive/????-??-??-<name>/UAT.md` under the main checkout root (the change root for archived targets) — the date-anchored pattern is the exact match (archive directories are `<YYYY-MM-DD>-<slug>`), so it cannot catch a different slug that merely ends in `-<name>`. A named archive entry wins even if a different change is active. If neither location has the file, **fail**: state that no UAT document was found for `<name>` and list both searched paths (`<change-root>/spec/changes/<name>/UAT.md` and the main checkout's `spec/archive/????-??-??-<name>/UAT.md`). Spawn nothing.
   - **No argument**: run `metta status --json` (Bash) to enumerate active changes; keep only those whose `<change-root>/spec/changes/<name>/` (each change's root resolved as above) contains a `UAT.md`. Exactly one candidate → select it. **Multiple candidates → fail with the candidate list** (never guess). Zero candidates → `Glob spec/archive/*/UAT.md` under the main checkout root, sort the parent directory names **descending** (names are `<YYYY-MM-DD>-<slug>`, so lexicographic sort is chronological; ties break by full-name sort, deterministic), take the first. Nothing anywhere → **fail** listing the searched locations (`spec/changes/*/UAT.md`, `spec/archive/*/UAT.md`); spawn nothing, create nothing.

2. **Snapshot for the post-run check.** Run `git -C "<change-root>" status --porcelain -- "<path>"`. If the target already has local modifications, **warn and stop** — a dirty target makes the post-run diff sanity check meaningless.
...
4. **Post-run diff sanity check.** Run `git -C "<change-root>" diff -- "<path>"` and verify the change is confined to the sanctioned regions:
   ...
   Then run `git -C "<change-root>" status --porcelain` over the **entire worktree** and require that the ONLY modified path is the target `UAT.md`. Any other modified or newly created tracked file → **do not commit**; report it as an unsanctioned runner write and stop.

5. **Commit.** Orchestrator only (the runner is contractually forbidden from git). Exact form:
   ```
   git -C "<change-root>" add "<path>" && git -C "<change-root>" commit -m "docs(<change-name>): UAT run record" -- "<path>"
   ```
   where `<change-name>` is the resolved change slug (archive slug without the date prefix for archived runs). The trailing `-- "<path>"` pathspec is mandatory: the commit MUST contain only the target `UAT.md` path, so pre-staged unrelated changes cannot ride along. Always `git -C "<change-root>"` with the paths quoted, never plain git from your cwd — for a worktree-hosted change plain git would target the wrong checkout or fail with 'outside repository'.
