...
1. Use `AskUserQuestion` to pick one of: `view`, `add`, `reorder`, `next`.
2. Dispatch per choice:
   ...
   - **next** → first run `metta roadmap --json` (allow-listed mint-cycle primer; also shows which entry is on top). Then run `metta roadmap next`. The CLI prints the `metta propose "<title>"` command to run next; echo that back to the user.

3. Echo the slug / position / next command printed by the CLI.

## Rules

- Never invent slugs; always use the ones emitted by the CLI (`metta backlog list --json` for `add`, `metta roadmap --json` for `reorder`).
